...
const EXAMPLE = {
  paragraph:
    'Good morning, Sara. You recorded two meals yesterday, so this reflection covers those meals rather than your full day. Your ناهار matched the plan closely, which is a steady start. Today begins with صبحانه, which has three options; when you are ready, you can log it from your plan and adjust the portions to what you actually eat. One meal at a time is a comfortable pace.',
...
export function reflectionSystemPrompt(): string {
  return [
    ...
    'Quote food and slot names exactly as they appear in the facts: as the user wrote them, in their language, with no translation and no English label added. For a slot with options, refer to the slot and, if useful, how many options it has; never tell the user which option to choose. If the plan has no meal times, refer to the next slot by plan order, never by a clock time.',
