 ...
 * completeness change; viewing a day never creates one. The read model lives
 * in `day-view.service.ts`.
 */
...
export async function markSlotSkipped(
  ...
  if (skipped) {
    ...
    const day = await ensureDayRecord(ownerId, localDate, await ownerZone(ownerId));
...
export async function setDayCompleteness(
  ...
  await prisma.dayRecord.upsert({
    ...
    create: {
      userId: ownerId,
      localDate,
      timeZone: await ownerZone(ownerId),
