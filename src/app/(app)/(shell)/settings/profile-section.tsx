...
/** Read view with an inline edit form (product spec § 5: values editable later). */
export function ProfileSection({ profile }: { profile: ProfileValues }) {
  ...
  if (!editing) {
    return (
      <div className="rounded-xl border border-border bg-card p-4" data-testid="profile-summary">
        <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
          ...
          <Row
            ...
            value={profile.sex ? t(`onboarding.sex.${profile.sex}`) : t('settings.profile.notSet')}
          />
          <Row
...
function Row({ label, value, bidi }: { label: string; value: string; bidi?: boolean }) {
  return (
    <>
      ...
      <dd className="font-medium">{bidi ? <bdi>{value}</bdi> : value}</dd>
    </>
  );
}
