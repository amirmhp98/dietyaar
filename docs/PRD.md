...
- `Plan`, `PlanSlot`, `PlanOption`, `PlanItem` (`unit` a measure, `unitGrams` for counts), `PlanTarget`, `PlanRule`, `PlanNote`, `PlanImportJob` — the one plan and its pending draft (`draftJson`, `draftRevision`)
