...
| `/onboarding`                                           | signed-in | One question per screen: age (under-18 stop deletes the account), sex, height, weight (metric/imperial), display name; resumable from `Profile.onboardingStep`; `?step=` reopens an answered question (Back from the plan step). Progress counts 10 steps: five questions, plan text, meals, targets, rules, ready |
...
| `/settings`                                             | signed-in | Profile (edit), preferences (appearance, units, week start), account (change password, log out), privacy text, export zip, delete account                                                                                                                                                                          |
