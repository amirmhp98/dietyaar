...
**FoodItem**: `mealId`, `position`, `originalName`, `englishLabel`, `quantity` nullable, `unit` nullable, `quantityUnknown` bool, `quantityAssumed` bool, `preparation` nullable, `category` enum, `alternatives` JSON, `matchedPlanItemId` nullable, `isAddedItem` bool, `restrictionHit` text nullable, `ruleGroups` text[] (user-confirmed food-group memberships for `DISTINCT_GROUPS` rules), `nutrition` JSON (section 5.1).
